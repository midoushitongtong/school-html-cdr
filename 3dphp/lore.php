<?php 
  ...
  _page("SELECT id FROM 3d_posts WHERE post_status = 'publish'AND
                            id in(SELECT object_id FROM 3d_term_relationships WHERE term_taxonomy_id = 8)",15);
  $_result = _query("SELECT
                      ...
                      WHERE
                            post_status = 'publish'
                        AND
                            id in (SELECT object_id FROM 3d_term_relationships WHERE term_taxonomy_id = 8)
                   ORDER BY
                            post_modified
                       DESC
                            LIMIT
...
<head>	
	...
	<link type="text/css" rel="stylesheet" href="style/lore.css">
				...
				<div>
					...
					<?php 
                      ...
                      while(!!$_rows = _fetch_array_list($_result)){
                          ...
                          echo '<li><a href="lore-content.php?id='.$_htmllist['id'].'">'._title($_htmllist['title'],23.3).'</a><strong><h3 class="rea">('._title($_htmllist['read'],2).')</h3> 阅读</strong><em>'.$_htmllist['date'].'</em></li>';                       
                      }
