...
$_result = _query("SELECT
                    ...
                    WHERE
                          post_status = 'publish'
                      AND      
                          id in(SELECT object_id FROM 3d_term_relationships WHERE term_taxonomy_id = 10 or term_taxonomy_id = 12)
                 ORDER BY
                          comment_count
                          DESC
                    LIMIT
					...
					<ul>
					 <?php 
                        ...
                        while(!!$_rows = _fetch_array_list($_result)) {
                          ...
                          echo '<li><a href="news-content.php?id='.$_htmllist['id'].'">'._title($_htmllist['title'],11).'</a><em class="hot">('._title($_htmllist['rea'],2).') 阅读</em></li>';
                        }
                      ?>
					</ul>
				</div>
