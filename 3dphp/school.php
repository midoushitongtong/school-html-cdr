<?php 
  define('IN_TG',true);
  define('SCRIPT','school');
  require dirname(__FILE__).'/includes/common.inc.php';
  _page("SELECT id FROM 3d_posts WHERE post_status = 'publish' AND
                              id in(SELECT object_id FROM 3d_term_relationships WHERE term_taxonomy_id = 16)",12);
  $_result = _query("SELECT
                      ...
                      WHERE
                            post_status = 'publish'
                        AND
                            id in (SELECT object_id FROM 3d_term_relationships WHERE term_taxonomy_id = 16)
                   ORDER BY
                            post_modified
                            DESC
                      LIMIT
			...
			<div id="left">
				...
				<div>
					<ul>
						...
						<li class="bg"><a href="school.php">教学环境</a></li>
					</ul>
				</div>
			</div>
			<div id="center">
